...
if (isset($_POST['register'])){
	$employee_number= mysqli_real_escape_string($mysqli,$_POST['employee_number']);
		$sql = "SELECT * FROM tech_info WHERE employee_id = '$employee_number'";
		...
		if(isset($active) && $active){ //ROCKY: check if something has been assigned to $active
			...
			if( !headers_sent() ){
             header("Location: http://localhost/Lab/techpage.php");
       }else{
  ?>
  <script type="text/javascript">
  document.location.href="http://localhost/Lab/techpage.php";
  </script>
  Redirecting to <a href="http://localhost/Lab/techpage.php">http://localhost/Lab/techpage.php</a>
  <?php
}
die();
